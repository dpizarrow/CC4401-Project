      ...
      xhr.onreadystatechange = function(){
          if(xhr.readyState===4 && xhr.status===200){
            ...
            for(i in response){
              ...
              row+="<td><a href=review_pelicula?id="+response[i]['id']+">Click para hacer una review</a></td>";
              row+=`<td>
              <div class="dropdown">
              ...
              playlist=JSON.parse(response[i]["playlist"])
                ...
                for(p in playlist){    
                    row+='<li><a class="dropdown-item" onclick="playlist_add_pelicula('+response[i]['id']+','+playlist[p]['pk']+')">'+playlist[p]['fields'].nombre+'</a></li>'
                  };
                  row+=`
